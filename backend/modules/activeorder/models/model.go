...
type GetOrderById struct {
	...
	Value, OrderId uint
	Ordered_at, Activated_at time.Time 
}
...
const GET = `select * from(
					...
					from orders as o
 					...
 					where isActivated = true AND isFinished = false
					order by o.order_id desc
					) smth
`
...
const GET_BY_ID = `
					select 
						...
						s.firstname || ' ' || s.lastname as SenderFullname, s.passport, s.phone, s.email, s.address, s.city, s.postcode,
 						o.order_id, o.comment, o.value, o.ordered_at, o.activated_at
 					from items as i
 					join orders as o using(order_id)
 					join receivers as r on r.receiver_id = o.receiver_id
 					join senders as s on s.sender_id = o.sender_id
 					where i.order_id = $1 AND isActivated = true AND isFinished = false
...
const COMPLETED = `
					update orders set (isFinished, finished_at) = (true, now()) where order_id = $1
`